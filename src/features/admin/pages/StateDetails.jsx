...
import { FiArrowLeft, FiEdit2, FiMap, FiCheckCircle, FiStar, FiImage } from "react-icons/fi";
...
const StateDetails = () => {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-slate-500">Loading state details...</div>;
  }

  if (isError || !stateData) {
    return <div className="p-8 text-center text-red-500">Error loading state.</div>;
  }
  ...
  const actions = (
    <>
      ...
      <button
        onClick={() => navigate(`/admin/states/edit/${id}`)}
  ...
  return (
    <AdminPageLayout
      ...
      subtitle={`Detailed view and management for ${stateData.name}`}
      actions={actions}
    >
          ...
          <div className="bg-white dark:bg-[#0A121F] border border-slate-200/80 dark:border-slate-800/40 rounded-3xl overflow-hidden shadow-sm">
            <div className="aspect-[4/3] bg-slate-100 dark:bg-slate-800 relative">
              ...
              <div className="absolute top-4 right-4 flex flex-col gap-2">
                <span className={`px-3 py-1 rounded-full text-xs font-black shadow-lg ${stateData.isActive ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'}`}>
                  {stateData.isActive ? "ACTIVE" : "HIDDEN"}
                </span>
                {stateData.featured && (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 text-slate-600 dark:text-slate-300">
                ...
                <div>
                  ...
                  <p className="font-bold capitalize">{stateData.region || "Unknown"}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 text-slate-600 dark:text-slate-300">
                <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[#E85D04] shrink-0">
                ...
                <div>
                  <p className="text-xs text-slate-400 font-semibold uppercase">Capital</p>
                  <p className="font-bold">{stateData.capital || "Unknown"}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-[#0A121F] p-6 sm:p-8 border border-slate-200/80 dark:border-slate-800/40 rounded-3xl shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-black text-slate-900 dark:text-white">State Overview</h2>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-sm italic">
                  "{stateData.tagline || "No tagline provided."}"
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-sm">
                  {stateData.description || "No description provided."}
                </p>
              </div>
            </div>
          </div>
