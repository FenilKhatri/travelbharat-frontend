...
import { FiBell, FiCheckCircle, FiInfo, FiAlertCircle, FiTrash2, FiCheck } from "react-icons/fi";
...
const Notifications = () => {
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (id) => {
      await http.put(`/notifications/admin/${id}/read`);
    },
    onSuccess: () => {
  ...
  const markAllAsReadMutation = useMutation({
    mutationFn: async () => {
      await http.put("/notifications/admin/read-all");
    },
    onSuccess: () => {
    ...
    <div className="space-y-6 pb-12 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Review recent system events and alerts.</p>
        </div>
        {unreadCount > 0 && (
          <button
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/40">
            {notifications.map((notif, index) => (
              <motion.div
                ...
                key={notif._id}
                className={`p-5 flex gap-4 transition hover:bg-slate-50 dark:hover:bg-slate-900/50 ${!notif.read ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-sm mt-1 ${!notif.read ? 'text-slate-600 dark:text-slate-400 font-medium' : 'text-slate-500 dark:text-slate-500'}`}>
                    {notif.message}
                  </p>
                </div>
                <div className="shrink-0 flex items-center gap-2 pl-4">
                  {!notif.read && (
                    <button
                      ...
                      title="Mark as read"
                      className="p-1.5 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition"
                    >
                      <FiCheck size={16} />
                    </button>
                  )}
